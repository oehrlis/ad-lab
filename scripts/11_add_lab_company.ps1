...
Write-Host
Write-Host "==============================================================" 
Write-Host "Start $ScriptName on host $Hostname at" (Get-Date -UFormat "%d %B %Y %T")

# call Config Script
if ((Test-Path $ConfigScript)) {
    Write-Host "load default values from $DefaultPWDFile"
    . $ConfigScript
} else {
...
if (-not (Get-ADOrganizationalUnit -Filter "Name -eq '$People'" -SearchBase $domainDn -ErrorAction SilentlyContinue)) {
    ...
    Write-Host "Done adding LAB company organisation -------------------------" 
} else {
    Write-Host "OU '$People' already exists in path '$domainDn'. Skip load of LAB company organisation."
}


Write-Host "Finish $ScriptName $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
Write-Host "==============================================================" 
# --- EOF ----------------------------------------------------------------------